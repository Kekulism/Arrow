...
vec4 effect(vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords)
{
    ...
    float front_x = local_x * (1 + (DISTANCE_FACTOR * lerp));
    vec2 front_coords = vec2(front_x * x_scale + local_x_origin, texture_coords.y);
    vec4 tex_front = Texel(texture, front_coords);
    if(front_coords.x < local_x_origin || front_coords.x > max_x) {
        tex_front = vec4(0.0);
    }

    vec2 back_coords = vec2(local_x, local_y); // SPINE
