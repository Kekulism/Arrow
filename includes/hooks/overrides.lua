...
local disable_empty = function()
    if not arrow_startup_item_check then
        arrow_startup_item_check = true
        for k, v in pairs(SMODS.ConsumableTypes) do
            if not G.FUNCS.arrow_consumabletype_has_items(k) then
                send('disabling '..k)
                ...
                if disable_if[k] then
                    for type, table in pairs(disable_if[k]) do
                        for i, key in ipairs(table) do
                            SMODS[type]:take_ownership(key, {
                                no_collection = true,
                                no_doe = true,
                                unlock_condition = nil,
                            }, true)
